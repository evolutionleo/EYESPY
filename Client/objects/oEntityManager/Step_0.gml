...
for(i = 0; i < l; i++) {
	...
	// if this is the local player - set the props, but nothing else
	if (instance_exists(MY_PLAYER) and uuid == MY_PLAYER.uuid) {
		// props
		var props = entity.p
		MY_PLAYER.dead = props.dead
		MY_PLAYER.color = props.color
		MY_PLAYER.hp = props.hp
		MY_PLAYER.max_hp = props.max_hp
		MY_PLAYER.name = props.name
		//MY_PLAYER.x = entity.x
		//MY_PLAYER.y = entity.y
	...
	var inst = find_or_create(uuid, type, , props, entity.x, entity.y)
